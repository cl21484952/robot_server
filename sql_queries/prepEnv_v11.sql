...
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkWaitingQueue` ()  proc:BEGIN
  ...
  WHERE
    -- q.groupID IN SELECT waitingQueue() AND    q.groupID IN
      (SELECT q.groupID
      FROM
  ...
  SELECT
    gID AS groupID,
    queueTicket AS queueNo,
    tableNum AS tableNo,
    positionID
  ;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `deleteSR` (IN `gID` VARCHAR(36) CHARSET utf8)  MODIFIES SQL DATA
BEGIN
DELETE FROM `servicereceiver`
WHERE groupID=gID;
...
-- Functions
--
CREATE DEFINER=`root`@`localhost` FUNCTION `amountCategory` (`amountOfPeople` INT) RETURNS INT(11) BEGIN
...
BEGIN
DECLARE qid varchar(36);
DECLARE qNo INT(4);
SET qNO = (SELECT COALESCE(MAX(queueNo), 0) FROM queue WHERE queueCate = cat)+1;
INSERT INTO queue(queueNo,queueDate,groupID,queueCate)
VALUES(qNO,SYSDATE(),groupid,cat);
...
BEGIN
    ...
    DECLARE queueCate varchar(1);
    SET queueCate = checkTableCategory(ppl);
    
    IF (queueCate='X' OR queueCate='Z') THEN
		RETURN queueCate;
    END IF;

    SET gID = srMake(ppl);
    SET queueNow = queueInsert(queueCate, gID);
    RETURN queueNow;
END$$
...
INSERT INTO `item` (`itemNo`, `itemName`, `itemDescription`, `itemPrice`, `isAvailable`, `category`) VALUES
(1, 'Fried Rice', 'Gud Fud', 45, 1, 1),
(2, 'Fried Egg', 'Gud Egg', 20, 1, 2),
(3, 'Lamb Sauce', 'Gordon Ramsy does not approve :c', 450, 0, 3);
...
INSERT INTO `queue` (`queueNo`, `queueDate`, `groupID`, `queueCate`) VALUES
(1, '2018-04-29 18:56:37', '5ee6c295-4ba4-11e8-b08a-0a002700000e', 'C'),
(1, '2018-04-29 18:57:00', '6c7926a5-4ba4-11e8-b08a-0a002700000e', 'B'),
(1, '2018-04-29 18:57:04', '6edbe73c-4ba4-11e8-b08a-0a002700000e', 'A');
...
('5323eec7-4ba4-11e8-b08a-0a002700000e', 6, '2018-04-29 18:56:17', NULL, 1),
('5ee6c295-4ba4-11e8-b08a-0a002700000e', 6, NULL, NULL, 1),
('634e24a9-4ba4-11e8-b08a-0a002700000e', 4, '2018-04-29 18:56:44', NULL, 1),
('6993473f-4ba4-11e8-b08a-0a002700000e', 2, '2018-04-29 18:56:55', NULL, 1),
('6c7926a5-4ba4-11e8-b08a-0a002700000e', 4, NULL, NULL, 1),
('6edbe73c-4ba4-11e8-b08a-0a002700000e', 1, '2018-04-29 18:57:06', NULL, 1),
...
INSERT INTO `sitsat` (`groupID`, `tableNo`) VALUES
('634e24a9-4ba4-11e8-b08a-0a002700000e', 1),
('6993473f-4ba4-11e8-b08a-0a002700000e', 2),
('6edbe73c-4ba4-11e8-b08a-0a002700000e', 3),
('ffea6b08-4858-11e8-b441-7a791935f7b4', 3),
('5323eec7-4ba4-11e8-b08a-0a002700000e', 4);
...
ALTER TABLE `queue`
  ...
  ADD KEY `queueCate` (`queueCate`) USING HASH,
  ADD KEY `queueNo` (`queueNo`);
...
--
ALTER TABLE `contain`
  ADD CONSTRAINT `contain_ibfk_1` FOREIGN KEY (`version`) REFERENCES `menu` (`version`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `contain_ibfk_2` FOREIGN KEY (`itemNo`) REFERENCES `item` (`itemNo`) ON DELETE CASCADE ON UPDATE CASCADE;
