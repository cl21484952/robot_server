...
--
-- Host: localhost:3306
-- Generation Time: May 01, 2018 at 07:49 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `checkWaitingQueue` ()  proc:BEGIN
  ...
  WHERE
      ...
      WHERE
        sr.enterDate IS NULL AND         sr.valid=1       ) AND
        rt.tableNo NOT IN
        (SELECT DISTINCT sa.tableNo         FROM `ServiceReceiver` sr, `SitsAt` sa
        WHERE sr.enterDate IS NOT NULL AND         sr.leaveDate IS NULL AND         sr.groupID=sa.groupID) AND     q.queueCate=checkTableCategory(rt.seatCount)
  ORDER BY
    q.queueDate ASC
  LIMIT 1
  ;
...
(5, 'Salad', 'Good salad', 20, 1, 1),
(6, 'Steak', 'Juicy', 20, 1, 1),
(7, 'Salmon', 'Good salmon', 20, 1, 1),
(8, 'pizzz', 'Good life', 20, 1, 1),
(10, 'MONT BLANC', 'Good life', 20, 1, 2),
(11, 'MOOOOSE CAKE', 'Good life', 20, 1, 2),
(12, 'Icecream', 'Good life', 20, 1, 2),
(13, 'Caramel machiato', 'Good life', 20, 1, 3),
(14, 'Orange Juice', 'Good life', 20, 1, 3),
(15, 'PERRIER', 'Good life', 20, 1, 3);
...
--
ALTER TABLE `item`
  MODIFY `itemNo` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=16;
--
